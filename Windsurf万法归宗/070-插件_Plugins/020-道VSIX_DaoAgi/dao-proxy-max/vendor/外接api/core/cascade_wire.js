...
//   实证依据: WINDSURF_AGENT_INJECTION_ROOT_ANALYSIS.md 行 695
//   stopVal=1 即"正常结束"; 其他值为预留扩展, 070 仅用 1 (END/END_TURN)
const STOP_END = 1;
const STOP_MAX_TOKENS = 2;
const STOP_TOOL_CALLS = 3;
const STOP_CONTENT_FILTER = 4;
const STOP_ERROR = 5;
...
/**
 * Connect-RPC 流末帧 · flags=2 · payload 是 JSON
 * - 正常结束: {}
 * - 异常: { error: { code, message } }
 */
function buildEndFrame(errMsg) {
  const json = errMsg
    ? JSON.stringify({ error: { code: "internal", message: errMsg } })
    : "{}";
...
function parseGetChatMessageRequest(reqBody, isJSON) {
  ...
  if (frames[0].flags & 1) {
    try { _payload = require("zlib").gunzipSync(_payload); } catch { return _emptyParsedReq(); }
  }
...
function _selfTest() {
  ...
  t("buildStopReasonFrame: 值=1 (END)", fpStop[RSP.STOP_REASON]?.[0]?.v === 1);
  ...
  t('buildEndFrame: payload="{}"', fEnd.slice(5).toString("utf8") === "{}");

  const fEndErr = buildEndFrame("boom");
  const errJson = JSON.parse(fEndErr.slice(5).toString("utf8"));
  t("buildEndFrame(err): 含 error.message", errJson?.error?.message === "boom");
