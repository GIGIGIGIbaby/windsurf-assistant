...
const ORIGIN_VERSION_BASE = "v9.9.53"; // v9.9.53 · 首尾互文双锚 · Turn Anchor 复归 · 承v9.9.52
...
// v9.9.53 · 首尾互文双锚 · Turn Anchor 复归 · 反者道之动
// 根因: 官方 SP 末段 ~1500 字裸文本闭幕辞 (engineering discipline) 被 invertSP 全删
//   模型读完 keeps 即入回答 · 无"此刻当为何"之行止嘱告 · 焦点偏移之根
// 本治: 帛书风格极小末锚 (~90字) · 首尾互文 (一章首锚"名可名也" + 末锚"道法自然之嘱")
//   位: keeps 之后最末 · 递归注意力权重最高处 · 顺 LLM 因果注意力本性
//   用: 焦点锚(当下所求) + 工具锚(召即来·并发) + 决策锚(明行疑问) + 界限锚(知止不殆)
//   不破: 帛书域纯净 · 无现代标识符 · 一字不入 XML · 子模型(invertAnySP)不加
// 道义: 八十一章「圣人无积·既以为人己愈有·天之道利而不害·人之道为而弗争」
//       四十七章「不出于户·以知天下」· 二十二章「少则得·多则惑」
const TAO_TURN_ANCHOR =
  "\n\n---\n\n道法自然之嘱:\n此刻用户所语为本 \u00b7 此前所述为镜 \u00b7 焦于当下所求\n工具者道之器 \u00b7 召之即来 \u00b7 无相依者并发于一\n所求若明 \u00b7 则行 \u00b7 所求若疑 \u00b7 则问\n不出于户 \u00b7 以知天下 \u00b7 知止可以不殆\n道法自然";

// v9.9.54 · TAO_SUB_ANCHOR · 副路轻量末锚 · summary/memory 专用
...
// 道义: 六十四章「为之于其未有也·治之于其未乱也」· 于乱未发时治之
//       二十二章「少则得·多则惑」· 极简才是真治
...
function _canonHeader(canon) {
  ...
  if (canon === "laozi+yinfu") {
    ...
    bookRef = entry ? entry.name : "\u5E1B\u4E66\u300A\u8001\u5B50\u300B"; // fallback: 帛书《老子》
  }
...
function invertSP(spText) {
  try {
    ...
    // v9.9.53 · 末锚: keeps 之后最末 · 首尾互文双锚 · 递归注意力权重最高
    return keeps
      ? base + TAO_TRAILER + keeps + TAO_TURN_ANCHOR
      : base + TAO_TURN_ANCHOR;
  } catch (e) {
    try {
...
function invertAnySP(spText) {
  try {
    ...
    //   有则保 · 无则简 · 名随实变.
    // v9.9.54 · 副路末锚 · summary/memory 加 TAO_SUB_ANCHOR · 断偏移传导链
    //   summary → conversation_summary → 下轮KEEP_BLOCKS → 主路 · 此传导链须在源头锚定
    //   ephemeral(标题/inline edit)不加: 任务极短 · 输出格式敏感 · 加锚无意义
    const keeps = extractKeepBlocks(s);
    const base = _canonHeader(_activeCanon) + _activeCanonText + TAO_FOOTER;
    if (t === "summary" || t === "memory") {
      return keeps
        ? base + TAO_TRAILER + keeps + TAO_SUB_ANCHOR
        : base + TAO_SUB_ANCHOR;
    }
