...
def _find_windsurf_dir():
    """Auto-detect Windsurf installation directory."""
    candidates = [
        r"D:\Windsurf",
        r"E:\Windsurf",
    ...
    for c in candidates:
        if os.path.exists(os.path.join(c, "resources", "app", "product.json")):
            return c
    # Glob fallback
    import glob
    for drive in ['C', 'D', 'E', 'F']:
        matches = glob.glob(drive + r':\**\Windsurf\resources\app\product.json', recursive=True)
        if matches:
