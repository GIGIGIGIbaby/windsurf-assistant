...
function Show-Status {
    ...
    if (Test-Path $INSTANCES_ROOT) {
        ...
        foreach ($inst in $instances) {
            ...
            $procId = if (Test-Path $lockFile) { (Get-Content $lockFile -ErrorAction SilentlyContinue).Trim() } else { "N/A" }
            $running = if ($procId -ne "N/A" -and (Get-Process -Id $procId -ErrorAction SilentlyContinue)) { "RUNNING" } else { "STOPPED" }
            ...
            Write-Host "  PID:      $procId" -ForegroundColor $(if ($running -eq "RUNNING") { "Green" } else { "Red" })
...
function Clean-Instance([int]$id) {
    ...
    # Check if running
    if (Test-Path $paths.LockFile) {
        $procId = (Get-Content $paths.LockFile -ErrorAction SilentlyContinue).Trim()
        if ($procId -and (Get-Process -Id $procId -ErrorAction SilentlyContinue)) {
            Write-Host "[ERROR] Instance $id is still running (PID $procId). Close it first." -ForegroundColor Red
            return
        }
    }
    ...
    if (Test-Path $paths.Base) {
        ...
        Get-ChildItem $paths.Base -Force -ErrorAction SilentlyContinue | ForEach-Object { Remove-Item $_.FullName -Force -ErrorAction SilentlyContinue }
        Remove-Item $paths.Base -Force -ErrorAction SilentlyContinue
