module.exports = function (PUBLIC_URL, HTTP_SCHEME) {
...
.agent-bar{display:flex;gap:6px;flex-wrap:wrap;margin-bottom:12px}
.agent-chip{padding:6px 14px;border-radius:8px;font-size:12px;cursor:pointer;border:1px solid #1a2040;background:#111828;color:#889;transition:all .2s;display:flex;align-items:center;gap:6px}
.agent-chip:hover{border-color:#7c8aff;color:#c0c8e0}
.agent-chip.active{border-color:#4caf50;background:#0d1a0d;color:#4caf50;font-weight:600}
.agent-chip .dot{width:6px;height:6px;border-radius:50%;background:#4caf50}
.agent-chip .admin{color:#ffa726;font-size:10px}
...
<div class="app">
  <div class="hdr">
    ...
    <div class="pills">
      ...
      <span id="aPill" class="pill off"><span class="d"></span>Agent <span id="aCount"></span></span>
    </div>
  </div>
  <div class="tabs">
  ...
  <div id="p-home" class="page act">
    <div id="agentBar" class="agent-bar" style="display:none"></div>
    <div id="agentCard" class="card">
      <h3>连接 Agent（远程之手）</h3>
      <p>在目标电脑以<b>管理员身份</b>打开 PowerShell，粘贴以下命令：</p>
      <div class="cmd-box" id="installCmd">irm ${HTTP_SCHEME}://${PUBLIC_URL}/agent.ps1 | iex</div>
...
function handle(m){
  ...
  else if(m.type==='agent_list'){
    renderAgentBar(m.agents,m.active);
  }
  else if(m.type==='agent_status'){
    agentOk=m.connected;
    if(m.connected){
      var label=m.hostname||'Agent';
      if(m.total>1) label+=' ('+m.total+')';
      aPill('on',label);
...
function renderAgentBar(list,active){
  var bar=document.getElementById('agentBar');
  if(!list||list.length===0){bar.style.display='none';return}
  bar.style.display='flex';
  var h='';
  list.forEach(function(a){
    var cls=a.active?'agent-chip active':'agent-chip';
    h+='<div class="'+cls+'" onclick="switchAgent(\''+esc(a.id)+'\')"><span class="dot"></span>'+esc(a.hostname||a.id);
    if(a.isAdmin) h+=' <span class="admin">admin</span>';
    h+='</div>';
  });
  bar.innerHTML=h;
  document.getElementById('aCount').textContent=list.length>1?'('+list.length+')':'';
}
function switchAgent(id){
  if(ws&&ws.readyState===1) ws.send(JSON.stringify({type:'switch_agent',agentId:id}));
}
