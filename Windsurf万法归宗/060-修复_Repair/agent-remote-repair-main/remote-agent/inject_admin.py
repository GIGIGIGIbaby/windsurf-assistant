"""
Inject session into Administrator's state.vscdb on DESKTOP-MASTER (141)
Try Firebase login with multiple strategies, then inject.
"""
import json, os, time, sqlite3, uuid, base64, ctypes, ctypes.wintypes, urllib.request, ssl

GS_DIR = r"C:\Users\Administrator\AppData\Roaming\Windsurf\User\globalStorage"
LS_PATH = r"C:\Users\Administrator\AppData\Roaming\Windsurf\Local State"
...
accts_path = os.path.join(GS_DIR, "undefined_publisher.windsurf-login-helper",
...
strategies = [
    ("direct", {}),
    ("7890", {"https": "http://127.0.0.1:7890"}),
    ("7897", {"https": "http://127.0.0.1:7897"}),
    ("10809", {"https": "http://127.0.0.1:10809"}),
    ("system", None),
]
...
if not id_token:
    print("\nAll Firebase strategies failed!")
    # Check network
    try:
        urllib.request.urlopen("https://www.google.com", timeout=5)
        print("  google.com: reachable")
    except Exception as e2:
        print(f"  google.com: {e2}")
    exit(1)
...
if key:
    try:
        ...
        print(f"v10 failed: {e}, trying DPAPI")
if not encrypted:
    try:
...
for fn in ["windsurf-auth.json", "cascade-auth.json"]:
    with open(os.path.join(GS_DIR, fn), "w") as f:
        json.dump(auth, f, indent=2)

print(f"\n{'='*50}")
print(f"SUCCESS: {display_name} ({email})")
print(f"Session + auth files written to Administrator profile")
print(f"{'='*50}")
