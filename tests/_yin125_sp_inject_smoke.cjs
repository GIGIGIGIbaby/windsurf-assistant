...
const PORT = 17781, BIND = "127.0.0.1";
...
async function dry(strategy, messages, extra) {
  return probe("POST", "/v1/system/sp-dryrun", H, Object.assign({ strategy, messages }, extra || {}));
}
...
async function main() {
  ...
  try {
    ...
    const setG = await probe("POST", "/v1/system/prompt", H, { globalSp: DAEMON_SP });
    if (setG.status === 200) ok("globalSp 设");
    else ng("globalSp 设", "status=" + setG.status);

    const setC = await probe("POST", "/v1/system/prompt", H, { customSp: CUSTOM_SP });
    ...
    const r400a = await probe("POST", "/v1/system/sp-dryrun", H, { strategy: "dao" });
    ...
    if (r400b.status === 400 && Array.isArray(r400b.json?.allowed)) ok(`非法 strategy → 400 allowed=${r400b.json.allowed.length}`);
    else ng("非法 strategy → 400", "实 " + r400b.status);

    // ── ④ bypass · 客 SP 原透 (无 daemonSp 加) ──
    console.log("\n\x1b[1m[四] bypass · 客端原透 (主公诏 SP §1)\x1b[0m");
    const r1 = await dry("bypass", [{ role: "system", content: "client-sp-原" }, { role: "user", content: "hi" }]);
    if (r1.status === 200 && r1.json?.output?.messages?.length === 2) {
      const sys = r1.json.output.messages[0];
      if (sys.role === "system" && sys.content === "client-sp-原") ok("bypass · 客 SP 原透");
      else ng("bypass · 客 SP", `sys.content=${sys.content?.slice(0, 40)}`);
      if (r1.json.output.meta.strategy === "bypass") ok("bypass · meta.strategy=bypass");
    ...
    if (r1b.json?.output?.messages?.length === 1 && r1b.json.output.messages[0].role === "user")
      ok("bypass · 无客 SP 时 · 不注入");
    else ng("bypass 无客 SP", "msgs=" + JSON.stringify(r1b.json?.output?.messages?.map(m => m.role)));
    ...
    if (r2.status === 200) {
      ...
      if (sys && sys.content?.length > 5000) ok(`dao · sys 注入 ${sys.content.length} 字 (≥5000)`);
      else ng("dao sys 长", "len=" + (sys?.content?.length || 0));
      if (sys?.content?.includes("请以下文《老子》之思想风格")) ok("dao · 含 TAO_HEADER 头");
      else ng("dao TAO_HEADER", "缺");
      if (sys?.content?.includes("反者") || sys?.content?.includes("道可道")) ok("dao · 含帛书真句");
      else ng("dao 帛书句", "缺");
      if (sys?.content?.includes("以上为风格指引")) ok("dao · 含 TAO_TRAILER 尾");
    ...
    if (r3.status === 200) {
      ...
      if (u?.content?.includes("<note name=\"dao-priority\"")) ok("usernote · 注入 <note name=\"dao-priority\">");
      else ng("usernote note 注入", "u.content head=" + u?.content?.slice(0, 60));
      ...
      if (r3.json.output.meta.usernoteInjected > 0) ok(`usernote · meta.usernoteInjected=${r3.json.output.meta.usernoteInjected}`);
    ...
    const r4 = await dry("prepend", [{ role: "system", content: "客SP" }, { role: "user", content: "hi" }]);
    const r4sys = r4.json?.output?.messages?.find(m => m.role === "system");
    if (r4sys?.content === DAEMON_SP + "\n\n客SP") ok("prepend · daemon + 客 (帛书三十六:友弱胜强)");
    else ng("prepend", "sys=" + r4sys?.content?.slice(0, 60));

    const r5 = await dry("append", [{ role: "system", content: "客SP" }, { role: "user", content: "hi" }]);
    ...
    const r6 = await dry("override", [{ role: "system", content: "客SP" }, { role: "user", content: "hi" }]);
    const r6sys = r6.json?.output?.messages?.find(m => m.role === "system");
    if (r6sys?.content === DAEMON_SP) ok("override · daemon 替客 (帛书四十:反者道之动)");
    ...
    const polluted = "<workspace_layout>污1</workspace_layout>真\n<MEMORY[abc]>污2</MEMORY[abc]>\n{\"mode\":\"SECTION_OVERRIDE_MODE_FOO\",\"content\":\"污3\"}";
    const r8 = await dry("bypass", [{ role: "user", content: polluted }]);
    if (r8.status === 200) {
      const u = r8.json.output.messages.find(m => m.role === "user");
      if (!u?.content?.includes("<workspace_layout>")) ok("stripSide · 移除 <workspace_layout>");
      else ng("stripSide", "未移");
      if (!u?.content?.includes("<MEMORY[")) ok("stripMem · 移除 <MEMORY[]>");
      else ng("stripMem", "未移");
      if (!u?.content?.includes("污3") || u?.content?.includes("道法自然")) ok("neutralize · SECTION_OVERRIDE 中和 (帛书一:道法自然)");
      else ng("neutralize", "未中和");
      if (u?.content?.includes("真")) ok("strip · 真内容保留");
      else ng("strip 真", "丢");
      if (r8.json.output.meta.strippedSide >= 1) ok(`meta.strippedSide=${r8.json.output.meta.strippedSide}`);
      else ng("meta strippedSide", "应 ≥ 1");
      if (r8.json.output.meta.strippedMem >= 1) ok(`meta.strippedMem=${r8.json.output.meta.strippedMem}`);
    ...
    if (stratBefore === stratAfter) ok(`dryrun 不动持久 · before=${stratBefore} after=${stratAfter}`);
    ...
    const tri = [
      { name: "OpenAI", method: "POST", path: "/v1/chat/completions", body: { model: "devin-cloud", messages: [{ role: "user", content: "hi" }] } },
      { name: "Anthropic", method: "POST", path: "/v1/messages", body: { model: "devin-cloud", max_tokens: 64, messages: [{ role: "user", content: "hi" }] } },
      { name: "Gemini", method: "POST", path: "/v1beta/models/devin-cloud:generateContent", body: { contents: [{ role: "user", parts: [{ text: "hi" }] }] } }
    ];
    for (const t of tri) {
      const r = await probe(t.method, t.path, H, t.body);
      // 无 token · 三协议都应至少不 500 panic · 返 200 SSE 错或 503/502/401 都 OK
      if (r.status >= 200 && r.status < 600 && r.status !== 500) ok(`${t.name} → ${r.status} (路由通 · 不 500)`);
  ...
  console.log("\n\x1b[1m═══ 印 125 总: \x1b[32m" + pass + " 过\x1b[0m\x1b[1m / \x1b[31m" + fail + " 失\x1b[0m\x1b[1m ═══\x1b[0m");
  if (fail > 0) {
    console.log("\n\x1b[31m失项:\x1b[0m");
