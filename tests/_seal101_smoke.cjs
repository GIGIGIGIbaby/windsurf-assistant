...
ok(appSrc.length > 80000, "§1.2 文 ≥ 80000 字 (got " + appSrc.length + " · 印 101 后应膨胀)");

// 用 Node 内置 require + 临时 wrapper 真解析 (IIFE 模式)
let parseOk = false;
try {
  // wrap in function 测语法 (不执行 DOM)
  new Function("daoSync", "window", "document", "navigator", "location", "AbortSignal", "crypto", appSrc);
  parseOk = true;
...
requiredFns.forEach((fn) => {
  ok(appSrc.includes(fn), "§2." + (requiredFns.indexOf(fn) + 1).toString().padStart(2, "0") + " " + fn);
});

// 全局状态
ok(/let __useTab\s*=\s*"chat"/.test(appSrc), "§2.x __useTab 默 'chat' (用户进默见对话)");
ok(/let __drawerOpen\s*=\s*null/.test(appSrc), "§2.y __drawerOpen 默 null (抽屉默收)");

// ───────────────────────────────────────────────────────────────────────
// §3 · enterMine 默走 v101 + ?v=100 fallback
// ───────────────────────────────────────────────────────────────────────
head("§3 enterMine: 默 v101 · ?v=100 旧三栏 fallback");
ok(
  /params\.get\("v"\)\s*===\s*"100"/.test(appSrc),
  "§3.1 enterMine 检 ?v=100 fallback",
);
ok(
  /\}\s*else\s*{\s*renderMineV101\(\);\s*\}/.test(appSrc),
  "§3.2 默走 renderMineV101 (用户进默 v101)",
);
ok(
  /renderLeft\(\);[\s\S]{0,80}renderMid\(\);[\s\S]{0,80}renderRight\(\);/.test(appSrc),
...
// script 引序 (印 100 已验过)
const order101 = /dao_github_sync\.js[\s\S]+?dao_bootstrap\.js[\s\S]+?dao_app\.js/;
ok(order101.test(html), "§4.4 script 引序: dao_github_sync → dao_bootstrap → dao_app (印 100 守)");
...
cssClasses.forEach((cls) => {
  ok(html.includes(cls), "§5." + (cssClasses.indexOf(cls) + 1).toString().padStart(2, "0") + " CSS " + cls);
});

// 抽屉折叠机制
ok(/\.v101-drawer\s*{[^}]*max-height:\s*0/.test(html), "§5.x 抽屉默 max-height: 0 (收)");
ok(/\.v101-drawer\.open\s*{[^}]*max-height:\s*55vh/.test(html), "§5.y 抽屉 .open 时 max-height: 55vh");
...
ok(/syncSpModeToVm/.test(appSrc), "§6.4 SP 三模 (passthrough/dao/custom) 同步到 VM");
ok(/存为模板/.test(appSrc), "§6.5 SP 库存模板");

// ③ 反代 API 管理 → 顶栏 + renderDrawer_endpt
ok(/复 Base URL/.test(appSrc) && /vmUrl \+ "\/v1"/.test(appSrc), "§6.6 ③ API 管理: 复 Base URL 一笔");
...
ok(/probeCloudFleet/.test(appSrc), "§6.9 云端 daemon 池入号节 (印 95/100 统一)");

// ⑤ agent 交互测试 → renderUseTab_batch
ok(/D\.batch\b/.test(appSrc) && /D\.batch\.prompts/.test(appSrc), "§6.10 ⑤ agent 测试: 批跑题集");
ok(/通过率:/.test(appSrc), "§6.11 通过率统计");
ok(/v1\/chat\/completions/.test(appSrc), "§6.12 批跑走 vmUrl /v1/chat/completions");
...
ok(/let __useTab\s*=\s*"chat"/.test(appSrc), "§7.3 默 chat tab (用户进默见对话)");
// 默 抽屉收 (用 80% / 管 默隐)
ok(/let __drawerOpen\s*=\s*null/.test(appSrc), "§7.4 默 抽屉收 (用 80% 屏 · 管 按需)");

// 顶栏一行三态 (反代活/号/模型)
ok(/反代活/.test(appSrc) && /反代未设/.test(appSrc), "§7.5 顶栏 ① 反代活否");
ok(/号: /.test(appSrc) && /accts\.length/.test(appSrc), "§7.6 顶栏 ② 当前号");
ok(/curModel/.test(appSrc) && /isB \?/.test(appSrc), "§7.7 顶栏 ③ 当前模型 (A/B 路染色)");

// ───────────────────────────────────────────────────────────────────────
// §8 · 道义守
// ───────────────────────────────────────────────────────────────────────
head("§8 道义守 (帛书四十八/三十二/六十四)");
ok(/为道者日损/.test(appSrc) || /日损之/.test(appSrc), "§8.1 帛书四十八「为道者日损」在 dao_app.js");
ok(/民莫之令而自均/.test(appSrc) || /民莫之令/.test(appSrc), "§8.2 帛书三十二「民莫之令而自均」在");
ok(/图难于其易/.test(appSrc) || /图难/.test(appSrc), "§8.3 帛书六十四「图难于其易」在");
ok(/反者道之动/.test(appSrc), "§8.4 「反者道之动」在 (主公诏)");
ok(/万法归宗/.test(appSrc), "§8.5 「万法归宗」立印");
ok(/大道至简/.test(appSrc), "§8.6 「大道至简」立印");
// 不破印 100 自举
ok(/印 100/.test(appSrc) || /印100/.test(appSrc), "§8.7 印 100 仍引 (反向兼容承)");
// 不破业务函
ok(/sendChat\b/.test(appSrc) && /addAccount\b/.test(appSrc), "§8.8 业务函 sendChat / addAccount 复用");
ok(/probeAccount\b/.test(appSrc) && /probeAll\b/.test(appSrc), "§8.9 业务函 probeAccount / probeAll 复用");
ok(/syncSpModeToVm\b/.test(appSrc) && /testVm\b/.test(appSrc), "§8.10 业务函 syncSpModeToVm / testVm 复用");
...
if (failed > 0) {
  ...
  console.log("✓ 印 101 smoke 全通 · 万法归宗 · 大道至简 · 用 + 管 · 反者道之动");
  console.log("  帛书·四十八: 为道者日损 · 损之又损 · 以至于无为 · 无为而无不为");
  console.log("  帛书·三十二: 侯王若能守之 · 万物将自宾 · 民莫之令而自均焉");
  console.log("  帛书·六十四: 图难于其易 · 为大于其细 · 圣人终不为大 · 故能成其大");
}
