...
// 态 2a: 有效 30 天 (绿)
{
  ...
  expect("tooltip 含 '到期:'", tag.includes("到期:"));
  expect("tooltip 含秒级 HH:mm:ss", /\d{2}:\d{2}:\d{2}/.test(tag));
...
console.log(
  "\n[D] v3.0 · _cleanseHealthOnLoad 无为 no-op · 不再清 Trial+planEnd=0",
);
{
  ...
  expect("trial_dirty 不清洗", report.trialNoPlanEnd === 0);
  expect(
    "trial_dirty.checked 保持 true",
    store.health["trial_dirty@t.com"].checked === true,
  );
  expect(
    "trial_dirty 不写 _trialDirtyAt 标",
...
console.log("\n[G] v3.0 · _cleanseHealthOnLoad no-op · trial 变体也不清");
{
  const { Store } = ext._internals;
  const os_ = require("node:os");
  const tmpDir = require("node:path").join(
  ...
  const report = store._cleanseHealthOnLoad();
  expect("Team Trial 保持", store.health["team_trial@t.com"].checked === true);
  expect("Free Trial 保持", store.health["free_trial@t.com"].checked === true);
  expect(
    "小写 trial 保持",
    store.health["lowercase_trial@t.com"].checked === true,
  );
  expect("trialNoPlanEnd 计 0", report.trialNoPlanEnd === 0);
  try {
    require("node:fs").rmSync(tmpDir, { recursive: true, force: true });
  } catch {}
}
