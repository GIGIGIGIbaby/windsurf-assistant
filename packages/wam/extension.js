...
function _processHubStuck(data) {
  ...
  const _notifyCooldown = Math.max(
    10000,
    +_cfg("hubNotifyCooldownMs", HUB_NOTIFY_COOLDOWN) || HUB_NOTIFY_COOLDOWN,
  );
  const _notifyGlobalCd = Math.max(
    1000,
    +_cfg("hubNotifyGlobalCdMs", HUB_NOTIFY_GLOBAL_CD) || HUB_NOTIFY_GLOBAL_CD,
  );
  const _renotifyInterval = Math.max(
    30000,
    +_cfg("hubRenotifyIntervalMs", HUB_RENOTIFY_INTERVAL) ||
      HUB_RENOTIFY_INTERVAL,
  );
  ...
  if (data.stuckList && data.stuckList.length > 0) {
    for (const s of data.stuckList) {
      ...
      if (
        lastNotifyTs > 0 &&
        (s.level === "DEAD" || sinceLast < _renotifyInterval)
      )
        continue;
...
<div class="conv-backup-path" title="${_esc(backupDir)}">备份: ${_esc(backupDir)}</div>
  ...
  if (hub.streamingList && hub.streamingList.length > 0) {
    currentHtml = hub.streamingList
      .map((c) => {
        // v13.0: 标题三级兜底: hub标题 → _convTitleMap缓存 → shortId
        const _cachedTitle = c.uuid ? _convTitleMap[c.uuid] || "" : "";
        const shortT = _truncTitle(c.title || _cachedTitle || c.shortId, 25);
        const st =
          c.staleSec > 0
            ? (c.staleSec >= 60
                ...
                : c.staleSec + "s") + "前"
            : "";
        // v13.0: 状态标识区分 streaming/等待用户/warning等
        const _stateIcon = c.isAwaitingUser
          ? "&#9654;" // 等待用户输入(常规箭头)
          : c.state === "warning" || c.state === "stuck"
            ? "&#9888;" // 警告
            : "&#9654;"; // 流式输出中
        const _stateHint = c.isAwaitingUser
          ? ' <span style="opacity:.5;font-size:9px">…待回</span>'
          : c.state === "warning"
            ? ' <span style="color:#e5a;font-size:9px">停滞</span>'
            : "";
        return `<div class="cv-current"><span class="cv-streaming">${_stateIcon}</span> <b>${_esc(shortT)}</b>${c.sizeKB ? " · " + c.sizeKB + "KB" : ""}${st ? " · " + st : ""}${_stateHint}</div>`;
      })
      .join("");
  } else if (hub.current) {
    const c = hub.current;
    ...
    tryFetchPlanStatus(_regApiKey, { apiServerUrl: _regApiServerUrl })
      .then((q) => {
        if (q) {
          ...
          if (_isD0) {
            const _retickMs = Math.max(
              0,
              +_cfg("zeroQuotaRetickMs", 2000) || 0,
            );
            if (_retickMs > 0) {
              log(
...
// v13.0 · 对话标题缓存刷新 · 从最新3个备份批次_index.json加载 uuid→title
// 调用时机: 备份完成后 · 插件激活时
// 费时: 可忽略 (仅读文件 · 每次备份只执行一次)
...
class Engine {
  ...
  async _tick() {
    ...
    const isSoftExhausted =
      !isHardExhausted && !_hasCreditsActive && effQuota < threshold;
